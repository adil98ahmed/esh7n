...
class WeOptions extends StatefulWidget {
  final MaterialColor? color;
  const WeOptions({Key? key, this.color}) : super(key: key);

  @override
  _WeOptionsState createState() => _WeOptionsState();
}

class _WeOptionsState extends State<WeOptions> {
  Future<void> takeAction() async {
    var option = await context.read<AlertDialogProvider>().getPreference();
    if (option == null) {
      showMyDialog();
    } else {
      await context.read<WeProvider>().takePhoto(option);
      showCircularBar();
      await context.read<WeProvider>().readTextFromImage();
      Navigator.of(context).pop();
      if (context.read<WeProvider>().flag == true) {
        showFail();
      } else {
        showCopyOrCall();
      }
  ...
  Future<void> oneTime() async {
    ...
    if (option != '') {
      ...
      await context.read<WeProvider>().readTextFromImage();
      Navigator.of(context).pop();
      if (context.read<WeProvider>().flag == false) {
        Navigator.of(context).pop();
        showCopyOrCall();
      }
      ...
      builder: (BuildContext context) {
        return new AlertDialog(
          actions: [
            TextButton(
                onPressed: () {
                  FlutterClipboard.copy(context.read<WeProvider>().copyCode());
                  Navigator.of(context).pop();
                  showCopyDone();
                },
          ...
          content: Builder(
            builder: (_) {
              var code = context.read<WeProvider>().code;
              return Container(
                  child: Text(
                code,
                    ...
                    children: [
                      Container(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            context.read<WeProvider>().chooseOption('*555*');
                            await takeAction();
                          },
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            context.read<WeProvider>().chooseOption('*599*');
                            await takeAction();
                          },
                          style: ElevatedButton.styleFrom(
                              primary: Colors.white.withOpacity(.8)),
                      ...
                      Container(
                        ...
                        child: ElevatedButton(
                          onPressed: () async {
                            context.read<WeProvider>().chooseOption('*566*');
                            await takeAction();
                          },
                          style: ElevatedButton.styleFrom(
                              primary: Colors.white.withOpacity(.8)),
